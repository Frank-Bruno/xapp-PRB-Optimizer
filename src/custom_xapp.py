...
class XappNori:
    ...
    def __init__(self):
        ...
        self.logger = Logger(name="XappNori", level=Level.DEBUG) # The name is included in each log entry, Levels: DEBUG < INFO < WARNING < ERROR
    ...
    def log_gnbs(self):
        ...
        for nbid in nbid_list:
            ...
            self.logger.info(f"Health Check Timestamp Received: {nbid.health_check_timestamp_received}") # 0 ???
            self.logger.info(f"Health Check Timestamp Sent: {nbid.health_check_timestamp_sent}") # 0 ???

            nbinfo = self._rmrxapp.GetNodeb(nbid.inventory_name)
            self.logger.info(f"Associated E2T Instance Address: {nbinfo.associated_e2t_instance_address}") # E2Term IP:PORT
            self.logger.info(f"E2 Application Protocol: {nbinfo.e2_application_protocol}") # 0 ???
            self.logger.info(f"Failure Type: {nbinfo.failure_type}") # 0 ???
            self.logger.info(f"Node Type: {nbinfo.node_type}") # 2 ????
            self.logger.info(f"RAN Name: {nbinfo.ran_name}") # Same as the inventory name
            self.logger.info(f"Setup From Network: {nbinfo.setup_from_network}") # True ??
            
            gnb = nbinfo.gnb
            self.logger.info(f"GNB Type: {gnb.gnb_type}") # 1 ????
            self.logger.info(f"Served NR Cells: {gnb.served_nr_cells}") # No cells informed
            
            self.logger.info("NodeB Configurations:")
            i = 1
            for config in gnb.node_configs:
                self.logger.info(f"Configuration {i}")
                self.logger.info(f"e2nodeComponentInterfaceType: {config.e2nodeComponentInterfaceType}")
                self.logger.info(f"e2nodeComponentInterfaceTypeE1: {config.e2nodeComponentInterfaceTypeE1}")
                self.logger.info(f"e2nodeComponentInterfaceTypeF1: {config.e2nodeComponentInterfaceTypeF1}")
                self.logger.info(f"e2nodeComponentInterfaceTypeNG: {config.e2nodeComponentInterfaceTypeNG}")
                self.logger.info(f"e2nodeComponentInterfaceTypeS1: {config.e2nodeComponentInterfaceTypeS1}")
                self.logger.info(f"e2nodeComponentInterfaceTypeW1: {config.e2nodeComponentInterfaceTypeW1}")
                self.logger.info(f"e2nodeComponentInterfaceTypeX2: {config.e2nodeComponentInterfaceTypeX2}")
                self.logger.info(f"e2nodeComponentInterfaceTypeXn: {config.e2nodeComponentInterfaceTypeXn}")
                self.logger.info(f"e2nodeComponentRequestPart: {config.e2nodeComponentRequestPart}")
                self.logger.info(f"e2nodeComponentResponsePart: {config.e2nodeComponentResponsePart}")
                i+=1
            ...
            for ran_func in gnb.ran_functions:
                ...
                self.logger.info(f"RAN Function Revision: {ran_func.ran_function_revision}")
                self.logger.info(f"RAN Function OID: {ran_func.ran_function_oid}")
                self.logger.info(f"RAN Function Definition: {ran_func.ran_function_definition}")     
    ...
    def subscribe_to_e2_nodes(self):
        ...
        sub_trs_id = self._rmrxapp.sdl_get(namespace="xappnori", key="subscription_transaction_id")
        ...
        for node in e2_nodes:
            ...
            subscription_req = self.generate_subscription_request(node.inventory_name, sub_trs_id)
            ...
            if int(resp.status_code/100) != 2:
                self.logger.error(f"Failed to subscribe to node {node.inventory_name}. Status code: {resp.status_code}, reason: {resp.reason}")
                continue
            ...
            self._rmrxapp.sdl_set(namespace="xappnori", key="subscription_transaction_id", value=sub_trs_id+1) # Update sub_trs_id on SDL
    ...
    def send_ran_slicing_control(self, sst:bytes, sd:bytes, action:int, rmrxapp: RMRXapp, summary: dict, sbuf):
        ...
        rmrxapp.rmr_rts(sbuf, new_payload=coded_pdu, new_mtype=12040) # 12040 = RIC Control Request
    ...
    def resubscribe_handler(self, name:str, path:str, data:bytes, ctype:str):
        ...
        response = xapp_rest.initResponse(
            ...
            response="xApp ACKs resubscription request"
